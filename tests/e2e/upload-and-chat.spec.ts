...
test.describe('Upload and Chat', () => {
  ...
  test('upload TXT file and navigate to chat', async ({ page }) => {
    ...
    await fileChooser.setFiles(path.join(__dirname, '../fixtures/sample.txt'))
  ...
  test('send chat message and get response', async ({ page }) => {
    ...
    await expect(page.getByPlaceholder(/ask a question/i)).toBeVisible({ timeout: 10000 })

    await page.getByPlaceholder(/ask a question/i).fill('What is this document about?')
    await page.getByPlaceholder(/ask a question/i).press('Enter')

    await expect(
      page.locator('[data-testid="assistant-message"], .text-muted-foreground').first(),
